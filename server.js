...
if (!process.env.DATABASE_URL) {
    console.error('DATABASE_URL environment variable is required to connect to PostgreSQL.');
    process.exit(1);
}
...
const USE_IMGBB = Boolean(IMGBB_API_KEY);

if (USE_IMGBB) {
    console.log('ImgBB storage enabled for student photos.');
} else {
    console.warn('IMGBB_API_KEY not provided; using local disk storage for photos.');
}
...
if (!USE_IMGBB && !fs.existsSync(uploadsDir)) fs.mkdirSync(uploadsDir, { recursive: true });
...
const upload = multer({ 
    storage: USE_IMGBB ? multer.memoryStorage() : diskStorage,
...
async function handleExternalUpload(req, res, next) {
    ...
    try {
        ...
        console.error('Error uploading photo to ImgBB:', err);
        return res.status(500).json({ error: 'Failed to upload photo to external storage.' });
    }
}
...
async function initializeDatabase() {
    try {
        ...
        await pool.query(`ALTER TABLE students ADD COLUMN IF NOT EXISTS published BOOLEAN DEFAULT FALSE`);
        await pool.query(`ALTER TABLE students ADD COLUMN IF NOT EXISTS level TEXT`);
        await pool.query(`ALTER TABLE students ADD COLUMN IF NOT EXISTS exam_link TEXT`);
        await pool.query(`ALTER TABLE students ADD COLUMN IF NOT EXISTS max_marks DOUBLE PRECISION`);
        await pool.query(`ALTER TABLE students ADD COLUMN IF NOT EXISTS photo_delete_url TEXT`);

        const { rowCount } = await pool.query(`SELECT 1 FROM admins WHERE username = $1 LIMIT 1`, ['admin']);
        if (rowCount === 0) {
            ...
            db.get("SELECT id_number FROM students WHERE exam_number = ? AND id_number != ? LIMIT 1", [finalExamNumber, id_number], (checkErr1, checkRow1) => {
                ...
                const { photoUrl: newPhotoUrl, photoDeleteUrl: newPhotoDeleteUrl } = getPhotoDataForUpdate(req.file);
                    ...
                    [student_name, level || null, exam_link || null, JSON.stringify(finalSubjectsToStore), totalMarks, maxMarks, computedGrade, examDate, newPhotoUrl, newPhotoDeleteUrl, existingId],
                        ...
                        if (USE_IMGBB && newPhotoDeleteUrl && previousPhotoDeleteUrl && previousPhotoDeleteUrl !== newPhotoDeleteUrl) {
                            await deletePhotoFromImgBB(previousPhotoDeleteUrl);
                            }
                                    ...
                                    if (sameExamType) {
                                        ...
                                        db.run(
                                            ...
                                            async function(updateErr) {
                                                ...
                                                if (USE_IMGBB && duplicateUpdatePhotoDeleteUrl && sameExamType.photo_delete_url && sameExamType.photo_delete_url !== duplicateUpdatePhotoDeleteUrl) {
                                                    await deletePhotoFromImgBB(sameExamType.photo_delete_url);
                                                }
                                                recomputeGradesForCohort(exam_type, level || null);
                                                // Generate QR code using id_number (primary identifier)
                                                const qrUrl = `${req.protocol}://${req.get('host')}/result/${id_number}`;
    ...
    db.get("SELECT id_number, exam_number FROM students WHERE id = ?", [studentId], async (err, studentRow) => {
        ...
        db.run(updateQuery, params, async (err) => {
            if (err) {
                ...
                if (USE_IMGBB && updatePhotoDeleteUrl && currentRow.photo_delete_url && currentRow.photo_delete_url !== updatePhotoDeleteUrl) {
                    await deletePhotoFromImgBB(currentRow.photo_delete_url);
                }
...
app.delete('/api/students/:id', (req, res) => {
    ...
    db.get("SELECT id, photo_delete_url FROM students WHERE id = ?", [studentId], (err, row) => {
        ...
        db.run("DELETE FROM students WHERE id = ?", [studentId], async function(deleteErr) {
            ...
            if (USE_IMGBB && photoDeleteUrl) {
                await deletePhotoFromImgBB(photoDeleteUrl);
            }
